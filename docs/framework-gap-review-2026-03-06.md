...
### 车卡与角色生成
- **技能分配约束** 仍未真正校验
  - 现在只有 `pointBudgets` 字段，尚未验证玩家填入技能是否超预算
  - 也没限制职业技能与兴趣技能的分配边界
- **年龄修正** 未实现
  - `MOV` 年龄修正未接
...
### 数值约束与规则限制
- **Credit Rating 范围校验** 未落地
  - 职业模板已有 `creditRatingRange`
  - 但 `createCharacter` 还没阻止超出职业范围的 `creditRating`
- **技能值上下限与默认值表** 不完整
  - 目前只校验 0-99
...
- **NPC 卡 schema 与 JSON 数据** 还可继续增强
  - 目前后效状态存在于运行态，不在 NPC 原始 card 中持久化
  - 若后面要做跨场景持续关系，需要单独的 `npc runtime state` 存档结构
- **跟踪 / 偷窃后的延迟后果** 仍偏轻
  - 现在有即时态度/怀疑变化
  - 但尚未把“过几分钟发现失物”“转移路线”“叫人”做成时间触发器
- **动态援军到达** 未实现
